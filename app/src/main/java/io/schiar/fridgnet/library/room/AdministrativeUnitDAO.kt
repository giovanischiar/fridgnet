...
abstract class AdministrativeUnitDAO {
    ...
    open suspend fun insert(geoLocation: GeoLocation, administrativeUnit: AdministrativeUnit) {
        val administrativeUnitEntityID = insertOrUpdate(administrativeUnit = administrativeUnit) ?: return
        insert(
    ...
    @Query("SELECT * FROM GeoLocation " +
            "WHERE administrativeUnitGeoLocationsID = (SELECT id FROM AdministrativeUnit " +
    ...
    @Query("SELECT * FROM GeoLocation " +
            "WHERE administrativeUnitGeoLocationsID IN (SELECT id FROM AdministrativeUnit " +
    ...
    private suspend fun insertOrUpdate(administrativeUnit: AdministrativeUnit): Long? {
        ...
        if (storedAdministrativeUnitEntities.isEmpty()) {
            return insert(administrativeUnitEntity = administrativeUnit.toAdministrativeUnitEntity())
        }
        ...
        if (storedAdministrativeUnitEntities.size > 1) {
            ...
            val administrativeUnitID = storedAdministrativeUnitEntities.filter { administrativeUnitEntity ->
            ...
            return if (administrativeUnitID != 0L) {
                ...
                Log.d(
                    "Store AdministrativeUnit",
                    "Can't assume if $locality is in one of these counties: $multipleCounties, or $subAdminArea. Create a new AdministrativeUnit"
                )
                insert(administrativeUnitEntity = administrativeUnit.toAdministrativeUnitEntity(id = administrativeUnitID))
            }
        }
        ...
        if (storedAdministrativeUnitEntity.subAdminArea == null && subAdminArea != null) {
            Log.d(
                "Store AdministrativeUnit",
                "Assuming ${storedAdministrativeUnitEntity.locality} is in $subAdminArea. Updating stored administrativeUnit"
            )
        ...
        if (subAdminArea == null && storedAdministrativeUnitEntity.subAdminArea != null) {
            Log.d(
                "Store AdministrativeUnit",
                "Assuming ${administrativeUnit.locality} is in ${storedAdministrativeUnitEntity.subAdminArea}"
            )
            return storedAdministrativeUnitEntity.id
        }
        ...
        if (subAdminArea != storedAdministrativeUnitEntity.subAdminArea) {
            Log.d(
                "Store AdministrativeUnit",
                "Can't assume if $locality is in ${storedAdministrativeUnitEntity.subAdminArea} or $subAdminArea. Inserting a new AdministrativeUnit"
            )
            return insert(administrativeUnitEntity = administrativeUnit.toAdministrativeUnitEntity())
        }

        return if (storedAdministrativeUnitEntity.id == 0L) null else storedAdministrativeUnitEntity.id
    }
    ...
    abstract fun selectAdministrativeUnitWithGeoLocations(): Flow<List<AdministrativeUnitWithLocationsAndGeoLocations>>

    @Query(
        "SELECT * FROM AdministrativeUnit WHERE " +
                "AdministrativeUnit.locality is :locality AND AdministrativeUnit.adminArea = :adminArea"
    )
    abstract suspend fun selectAdministrativeUnitsBy(locality: String, adminArea: String): List<AdministrativeUnitEntity>
}